...
stats = assay + geno + rem + start #Concatenates strings from previous 4 regular expressions
proportion = int(stats[2])/int(stats[3])#Calculates a proportion of of variants removed by filter.
stats.append(proportion)#Appends to list of strings

time = re.findall(r'Start time: ([0-9 a-z A-Z : .]+)',ulog) #Regex to grab timestamp from .log file. Appends to list of strings
stats.append(time)
