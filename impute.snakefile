...
rule convert:
	...
	output:
		...
		log = "impute_input/{sample}.chr{chr}.phased.log"
	shell:
...
rule impute_input:
	...
	shell:
		"(perl bin/vcf2impute_legend_haps.pl -vcf {input.vcf} -leghap {params.oprefix} -chr {params.chrom}; perl bin/vcf2impute_gen.pl -vcf {input.vcf} -gen {params.genfix} -chr {params.chrom}) > {log}"
...
rule run_impute2_round1:
	input:
		...
		gens= expand("eagle_phased_assays/{sample}.chr{{chr}}.phased.haps", sample = IMPGENS1),
